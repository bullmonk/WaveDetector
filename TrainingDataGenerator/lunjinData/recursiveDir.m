...
%       found items.
%
% NOTE: Any words or pattern from IgnoreList will be applied on all folder
% levels, whereas Expression will only be applied on the last level. This
% behavior is open for discussion.
%
% EXAMPLES:
...
%   Written by Eivind Hennestad | v1.0.0
    ...
    arguments
        ...
        options.OutputType (1,1) string {mustBeMember(options.OutputType, {'FileAttributes', 'FilePath', 'RelativeFilePath'})} = 'FileAttributes'
    ...
    if numel(rootPath) > 1
        ...
        % Remove any items that contain a word from the ignore list.
        ...
        % 3. Only keep listed items that matches expression
        if options.Expression ~= ""
            isValidName = @(fname) ~isempty(regexp(fname, options.Expression, 'once'));
            isMatch = cellfun(@(name) isValidName(name), {filteredListing.name} );
            keep = keep & isMatch;
        end
        ...
        % 6. Recursively search through subfolders. NB. Here the filtered
        % listing is used, so we will still search through subfolders that
        % does not match the value of Expression.
    ...
    elseif outputType == "RelativeFilePath"
        combinedListing = getAbsPathName(combinedListing);  % Local function
        combinedListing = strrep(combinedListing, rootPath, '' );
    end
end
